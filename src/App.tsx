...
import { Search, Download, ChevronLeft, ChevronRight, Menu, X } from "lucide-react"
...
function App() {
  ...
  const sampleData: Transaction[] = Array(5)
    .fill(null)
    .map((_, idx) => ({
      fecha: "2025-09-17",
      ...
      monto: `$${(50.0 + idx * 25.5).toFixed(2)}`,
      montoComision: `$${(2.5 + idx * 0.75).toFixed(2)}`,
      montoIva: `$${(0.3 + idx * 0.09).toFixed(2)}`,
      montoNeto: `$${(47.2 + idx * 24.66).toFixed(2)}`,
      moneda: "USD",
      paisOrigen: "Ecuador",
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="p-3 lg:p-6 max-w-[1920px] mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-md p-4 lg:p-6 mb-6">
          ...
          {showMoreFilters && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4 mt-4 pt-4 border-t border-gray-200">
              <div>
                <label className="block text-xs lg:text-sm font-medium mb-1 text-gray-700">
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
          ...
          <div className="p-4 border-b border-gray-200 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3">
            <div className="flex flex-wrap items-center gap-2 text-xs lg:text-sm">
              <span className="font-semibold">
                Total de Registros: <span className="text-[#003B5C]">1000</span>
              </span>
              <span className="font-semibold">
                Monto: <span className="text-green-600">$ 3,000,000.00</span>
              </span>
            </div>
          ...
          {/* Responsive Table Container - Now shows table on all screen sizes */}
          <div className="overflow-x-auto">
            <div className="inline-block min-w-full align-middle">
              {/* Table - Now visible on all screen sizes */}
              <div className="overflow-auto" style={{ maxHeight: "600px" }}>
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-100 sticky top-0 z-10">
                    <tr>
                      {visibleColumns.fecha && (
                        <th className="px-3 lg:px-4 py-2 lg:py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap">
                          Fecha
                        </th>
                      )}
                      {visibleColumns.hora && (
                      ...
                      {visibleColumns.referenciaVoucher && (
                        <th className="px-3 lg:px-4 py-2 lg:py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap">
                          Ref. Voucher
                        </th>
                      )}
                      ...
                      {visibleColumns.autorizador && (
                        <th className="px-3 lg:px-4 py-2 lg:py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap">
                          Autorizador
                        </th>
                      )}
                      {visibleColumns.tipoTarjeta && (
                        <th className="px-3 lg:px-4 py-2 lg:py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap">
                          Tipo Tarjeta
                        </th>
                      )}
                      {visibleColumns.numeroLote && (
                        <th className="px-3 lg:px-4 py-2 lg:py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap">
                          N° Lote
                        </th>
                      )}
                      ...
                      {visibleColumns.estadoTransaccion && (
                        <th className="px-3 lg:px-4 py-2 lg:py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap">
                          Estado
                        </th>
                      )}
                      ...
                      {visibleColumns.direccionComercio && (
                        <th className="px-3 lg:px-4 py-2 lg:py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap">
                          Dirección
                        </th>
                      )}
                      {visibleColumns.telefonoComercio && (
                        <th className="px-3 lg:px-4 py-2 lg:py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap">
                          Teléfono
                        </th>
                      )}
                      {visibleColumns.emailComercio && (
                        <th className="px-3 lg:px-4 py-2 lg:py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap">
                          Email
                        </th>
                      )}
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {sampleData.map((row, idx) => (
                      <tr key={idx} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                        {visibleColumns.fecha && (
                          <td className="px-3 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm text-gray-900 whitespace-nowrap">
                            {row.fecha}
                          </td>
                        )}
                        {visibleColumns.hora && (
                        ...
                        {visibleColumns.midRed && (
                          <td className="px-3 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm text-blue-600 whitespace-nowrap">
                            {row.midRed}
                          </td>
                        )}
                        {visibleColumns.codigoComercio && (
                        ...
                        {visibleColumns.monto && (
                          <td className="px-3 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm font-semibold text-green-600 whitespace-nowrap">
                            {row.monto}
                          </td>
                        )}
                        {visibleColumns.montoComision && (
                        ...
                        {visibleColumns.montoNeto && (
                          <td className="px-3 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm font-semibold text-gray-900 whitespace-nowrap">
                            {row.montoNeto}
                          </td>
                        )}
                        {visibleColumns.moneda && (
                        ...
                        {visibleColumns.direccionComercio && (
                          <td className="px-3 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm text-gray-900 whitespace-nowrap">
                            {row.direccionComercio}
                          </td>
                        )}
                        {visibleColumns.telefonoComercio && (
                          <td className="px-3 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm text-gray-900 whitespace-nowrap">
                            {row.telefonoComercio}
                          </td>
                        )}
                        {visibleColumns.emailComercio && (
                          <td className="px-3 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm text-gray-900 whitespace-nowrap">
              ...
              {[1, 2, 3, 4].map((page) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded transition-colors text-sm ${
                    currentPage === page ? "bg-[#FF8C00] text-white" : "hover:bg-gray-100"
                  }`}
                >
                  {page}
                </button>
